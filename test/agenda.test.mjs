...
test('unverified tasks remain visible with a warning and Ink breaks only same-time ties',()=>{
 ...
 const result=buildAgenda([a,b],now);assert.equal(result.items[0].network,'ink');assert.equal(result.items[0].warnings.length,3);assert.equal(result.items.length,2);
});
