...
test('local agents use loopback model, save successful chat and leave projects untouched',async()=>{
 ...
 const a=new LocalAgents(s,async(url,options)=>{assert.equal(url,'http://127.0.0.1:11434/api/chat');const body=JSON.parse(options.body);assert.equal(body.model,'qwen3:4b');assert.equal(body.think,false);return Response.json({message:{content:'Потрібні докази.'}});});
